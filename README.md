# Hi there! 👋 I'm King Ofosu Hagan

### 📱 Aspiring Mobile App Developer | Flutter Enthusiast

Welcome to my corner of GitHub! I am a **passionate and driven student** currently on the exciting journey to become a professional Mobile App Developer. My primary focus and skill set revolves around **Flutter and the Dart language**, which I use to build beautiful, natively compiled, multi-platform applications.

---

## 🎯 About Me

* 🎓 **Student:** I am currently **Computer Science** at **Kwamena University of Science and Technology**.
* 💻 **Tech Focus:** My heart is set on **mobile development**. I am constantly learning and honing my skills in **Flutter, Dart, State Management (e.g., Provider, Bloc), UI/UX design principles, and Firebase integration.**
* 🌱 **Learning & Growth:** I thrive on challenges and continuous learning. I'm actively working on understanding efficient application architecture, writing clean, testable code, and exploring new technologies that enhance the mobile user experience.
* 💡 **Goal:** To transition from an aspiring student to a proficient Mobile App Developer, creating intuitive and high-quality applications that solve real-world problems.

---

## 🛠️ My Toolbox

Here are some of the technologies and tools I frequently use:

| Category | Tools & Technologies |
| :--- | :--- |
| **Mobile Framework** | **Flutter**, Dart |
| **State Management** | Provider, Bloc/Cubit (Learning) |
| **Backend & Database** | **Firebase** (Firestore, Authentication), REST APIs |
| **Languages** | Dart, HTML, CSS, JavaScript (Basics) |
| **Version Control** | Git, **GitHub** |
| **Tools/IDE** | VS Code, Android Studio |

---



## 📫 Let's Connect!

I'm always open to discussing new projects, learning opportunities, or just chatting about Flutter! Feel free to reach out.

| Platform | Link |
| :--- | :--- |
| **Email** | <a href="mailto:kingofosuhagan@gmail.com" target="_blank">✉️ kingofosuhagan@gmail.com</a> |
| **LinkedIn** | <a href="https://www.linkedin.com/in/king-ofosu-hagan-359462396" target="_blank">👔 LinkedIn</a> |

| **GitHub** | <a href="https://github.com/hagan-del" target="_blank">🐙 GitHub</a> |

